...
Grid.mixin({
	...
	getEventTimeText: function(range, formatStr, displayEnd) {
	    ...
	    //Modified by cgzhou, fix bug in calendar
...
DayGrid.mixin({
	...
	renderMoreLink: function(row, col, hiddenSegs) {
		...
		return $('<a class="fc-more"/>')
			.text(
				this.getMoreLinkText(hiddenSegs.length, reslicedAllSegs.length)
			)
	...
	getMoreLinkText: function(num, allSegsNum) {
		var opt = this.view.opt('eventLimitText');

		if (typeof opt === 'function') {
		    return opt(num, allSegsNum);
		}
		else {
			return '+' + num + ' ' + opt;
		}
